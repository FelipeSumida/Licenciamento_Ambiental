...
export function PdfProcesso({ processo }: PdfProcessoProps) {
  const situacao =
    ...
    <div className="hidden print:block pdf-page">
        <header className="pdf-header">
            ...
            <div className="pdf-header-text">
                <h1>Comunique-se</h1>
                <p>Relatório de Processo de Licenciamento Ambiental</p>
        ...
        <div className="pdf-process-highlight">
            <div>
                <span>NÚMERO DO EMPREENDIMENTO</span>
                    <strong>{processo.idEmpreendimento ?? "Sem número"}</strong>
                </div>
            ...
            <section className="pdf-section">
                <h2>Fases do trecho</h2>

                {fasesTrecho.length ? (
                <div className="pdf-items">
                    {fasesTrecho.map((fase: any, index: number) => (
                        <div className="pdf-item" key={fase.id ?? index}>
                            <h3>Fase {index + 1}</h3>
                            ...
                            <div className="pdf-grid">
                                ...
                                <PdfCampo
                                    label="Número"
                                    valor={fase.numeroFase}
                                />

                                <PdfCampo
                                    label="Data de emissão"
                                    valor={formatarData(fase.dataEmissaoFase)}
                                />

                                <PdfCampo
                                    label="Data de validade"
                                    valor={formatarData(fase.dataValidadeFase)}
                                />

                                <PdfCampo
                                    label="Anexo"
                                    valor={fase.anexoFase}
                                />
                            </div>
                        </div>
                    ))}
                    ...
                    <p className="pdf-empty">
                        Nenhuma fase do trecho registrada.
                    </p>
                )}
            </section>

            <section className="pdf-section">
                <h2>Fases complementares</h2>

                {processo.fasesComplementares?.length ? (
                    <div className="pdf-items">
                        {processo.fasesComplementares.map(
                            (fase: any, index: number) => (
                                <div className="pdf-item" key={fase.id ?? index}>
                                    <h3>Fase complementar {index + 1}</h3>
